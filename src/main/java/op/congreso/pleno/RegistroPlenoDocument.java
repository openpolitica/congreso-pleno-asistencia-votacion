...
public record RegistroPlenoDocument(
  ...
  public static StringBuilder csvHeader() {
    return new StringBuilder(
      "periodo_parlamentario,periodo_anual,legislatura," + "fecha,titulo,url,filename,paginas\n"
    );
  }
      ...
      for (var tr : trs) {
        if (tr.children().size() == 6) {
          ...
          if (matcher.find()) {
            ...
            root.put(titulo, new RegistroPlenoDocument(pp, pa, l, date, titulo, fullUrl, fullUrl.split("/")[9], 0));
          }
        }
      }
      return root;
